...
function ProductPage() {
  ...
  const fetchData = () => {
    axios
      .get("http://localhost:8080/pos/api/listproductall")
      .then((response) => {
  ...
  const handleDeleteProduct = (productId) => {
    axios
      .delete(`http://localhost:8080/pos/api/deleteproduct/${productId}`)
      .then(() => {
      ...
      .catch((error) => {
        Swal.fire({
          ...
          text: "You can't delete a product that has already been linked to transactions.",
        });
  ...
  return (
    <div className="p-8 bg-teal-100 rounded min-h-screen">
      <div className="flex justify-between ">
        <h1 className="text-3xl font-bold mb-6 text-teal-400">Product List</h1>
